...
const { computeSignupTodos, ageFromBirthDate, kidLacksAfternoonPick } = require('./_signup_todos');
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'signup-todos') {
        ...
        // The computation lives in api/_signup_todos.js (#368 follow-up) so
        // the To Do notification registry can run the same read.
        const todosOut = await computeSignupTodos(sql, stYear, stSess);
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'volunteer-signup') {
        ...
        touchTodos(sql, ['vp-adults','vp-assist']); // #368
      ...
      if (action === 'class-set-max') {
        ...
        touchTodos(sql, ['acl-overmax','acl-confirm']); // #368
      ...
      if (action === 'class-signup-window') {
        ...
        touchTodos(sql, ['vp-adults','vp-assist','kids-unpicked','acl-overmax','acl-confirm']); // #368
      ...
      if (action === 'class-signup-picks') {
        ...
        await sql.transaction(pickStmts);
        touchTodos(sql, ['kids-unpicked','acl-overmax','acl-confirm']); // #368
