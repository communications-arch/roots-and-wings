...
async function handleEventOpeningsGet(req, res) {
  ...
  try {
    ...
    `;
    // #79 follow-up: lead hand-raises live only in event_seat_interest
    // (no direct add), so the viewer's ✓ state and the "interested so
    // far" names come from the log itself.
    const handRows = ids.length ? await sql`
      SELECT event_id, seat, person_name, LOWER(person_email) AS em
      FROM event_seat_interest WHERE event_id = ANY(${ids})
      ORDER BY created_at, id
    ` : [];
...
// POST kind='bug-reopen' — a tester flags a "fixed" bug as still broken
// (Erin, 2026-07-22: "should we close it and open a new bug?" — no:
// this keeps the whole story on ONE issue). Comments their note and
// strips fixed-on-dev/verified so the card returns to "Reported".
async function handleBugReopen(body, req, res) {
  if (process.env.VERCEL_ENV === 'production') return res.status(404).json({ error: 'Not found.' });
  const auth = await verifyBugReporter(req);
  if (!auth) return res.status(401).json({ error: 'Unauthorized' });
  const token = process.env.GITHUB_BUGLOG_TOKEN;
  ...
  try {
    ...
    const issue = await cur.json();
    const labels = (issue.labels || []).map(l => String((l && l.name) || ''));
    if (issue.state !== 'open' || labels.indexOf('fixed-on-dev') === -1) {
      return res.status(409).json({ error: 'That one isn’t marked fixed right now — refresh the list.' });
    }
    const cm = await fetch(issueUrl + '/comments', {
      method: 'POST',
      headers: Object.assign({ 'Content-Type': 'application/json' }, bugLogGithubHeaders(token)),
      body: JSON.stringify({
        body: '❌ Still broken per ' + (auth.name || auth.email) + ' (' + auth.email + ') via the dev portal bug log.'
          + (note ? '\n\n' + note : '')
          + '\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_'
      })
    });
    if (cm.status !== 201) {
      console.error('Bug reopen comment error:', cm.status, await cm.text().catch(() => ''));
      return res.status(502).json({ error: 'Could not save that right now — please try again.' });
    }
    for (const lbl of ['fixed-on-dev', 'verified']) {
      try {
        await fetch(issueUrl + '/labels/' + encodeURIComponent(lbl), {
          method: 'DELETE',
          headers: bugLogGithubHeaders(token)
        });
      } catch (e) { /* label may not exist — fine */ }
    }
    bugListCache = null;
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('Bug reopen error:', err);
    return res.status(502).json({ error: 'Could not save that right now — please try again.' });
  }
}
