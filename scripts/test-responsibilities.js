...
t('Class Builder lives on Co-op Management only — no standalone pm-scheduling card', () => {
  ...
  // Group liaisons' fallback widget list: roles card + the #139 Snack
  // List editor + the #140 My Class roster card.
  assert.ok(/return \['todos', 'my-class', 'group-bring', 'roles'\];/.test(src2), "group-liaison fallback must return ['todos', 'my-class', 'group-bring', 'roles']");
});
